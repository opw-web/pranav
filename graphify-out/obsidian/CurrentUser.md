---
source_file: "lens/app/auth/deps.py"
type: "code"
community: "CurrentUser"
location: "L14"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/CurrentUser
---

# CurrentUser

## Connections
- [[archive_account()]] - `references` [EXTRACTED]
- [[bulk_action()]] - `references` [EXTRACTED]
- [[categories_page()]] - `references` [EXTRACTED]
- [[category_picker()]] - `references` [EXTRACTED]
- [[commit()]] - `references` [EXTRACTED]
- [[confirm()]] - `references` [EXTRACTED]
- [[create_account()]] - `references` [EXTRACTED]
- [[create_category_route()]] - `references` [EXTRACTED]
- [[create_txn()]] - `references` [EXTRACTED]
- [[delete_category_route()]] - `references` [EXTRACTED]
- [[delete_rule()]] - `references` [EXTRACTED]
- [[delete_txn()]] - `references` [EXTRACTED]
- [[deps.py]] - `contains` [EXTRACTED]
- [[edit_txn_row()]] - `references` [EXTRACTED]
- [[get_current_user()]] - `references` [EXTRACTED]
- [[get_scoped_session()]] - `references` [EXTRACTED]
- [[get_txn_row()]] - `references` [EXTRACTED]
- [[import_page()]] - `references` [EXTRACTED]
- [[learn()]] - `references` [EXTRACTED]
- [[list_accounts()]] - `references` [EXTRACTED]
- [[list_txn_page()]] - `references` [EXTRACTED]
- [[merge_categories_route()]] - `references` [EXTRACTED]
- [[patch_txn()]] - `references` [EXTRACTED]
- [[preview()]] - `references` [EXTRACTED]
- [[quickadd_commit()]] - `references` [EXTRACTED]
- [[quickadd_preview()]] - `references` [EXTRACTED]
- [[reconcile()]] - `references` [EXTRACTED]
- [[recurring_page()]] - `references` [EXTRACTED]
- [[rename_account()]] - `references` [EXTRACTED]
- [[rename_category_route()]] - `references` [EXTRACTED]
- [[root()]] - `references` [EXTRACTED]
- [[rules_page()]] - `references` [EXTRACTED]
- [[undo_delete_txn()]] - `references` [EXTRACTED]
- [[upload()]] - `references` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/CurrentUser